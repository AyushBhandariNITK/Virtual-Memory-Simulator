...
{
    vmTable_t* new_table = (vmTable_t*)malloc(sizeof(vmTable_t));
    new_table->length = length;
    new_table->pageNumArr = (int*)malloc(sizeof(int) * length);
    new_table->frameNumArr = (int*)malloc(sizeof(int) * length);
    new_table->entryAgeArr = (int*)malloc(sizeof(int) * length);
    ...
    if(new_table == NULL || new_table->pageNumArr == NULL || new_table->frameNumArr == NULL) {
        cout<<"Error - Could not allocate a new Virtual Memory Addressing Table!\r\n";
        exit(-1);
    }
    return new_table;
}
...
void displayTable(vmTable_t** tableToView)
{
    ...
    for (int i = 0; i < (*tableToView)->length; i++) {
        cout<<"Index("<<i<<") := Page Number: "<<(*tableToView)->pageNumArr[i]<<"\tFrame Number: "<<(*tableToView)->frameNumArr[i]<<"\n";
    }
    cout<<"\n********************* SEQUENCE END ***************************\n ";
}
...
{
    int** temp;
    temp = (int**)malloc(frameCount * sizeof(int*));
    ...
    if(temp == NULL) {
        cout<<"Error - Could not allocate a new Physical Memory Matrix using dramAllocate() function!\r\n";
        exit(-1);
    }
    return temp;
}
